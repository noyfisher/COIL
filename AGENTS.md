...
Full rules: `~/.Codex/projects/-Users-noyfisher-IOS-Projects-PT-Helper-Agent-v1/memory/feedback_engineering_protocol.md`
...
### AI Request Types
All AI calls go through `ClaudeAPIService` → Firebase Cloud Function proxy (`functions/src/index.ts`) → Codex API. System prompts and model config live server-side. The `AIRequestType` enum in `ClaudeAPIService.swift` has 9 cases:
...
|---|---|---|
| `claudeProxy` | HTTP | Routes AI requests to Codex API with rate limiting (20 req/min/user) |
