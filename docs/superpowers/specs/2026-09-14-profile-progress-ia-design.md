...
- The "only `GuidedWorkoutView` writes `isTabBarHidden`" rule was relaxed at the final review: `TabSelection.popToRootAndGoHome()` and `popToRootCurrentTab()` also clear it, because a pop-to-root rotates the stack's `.id()` and a skipped `onDisappear` would strand the user without a tab bar. The deep-link → pop-to-root path while a workout is up was not exercised on the simulator; if it ever shows a stale hidden bar, that is the place to look.
